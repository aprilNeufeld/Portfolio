 ...
 */
const getChips = (minChips: number, maxChips: number, minWidth: number, maxWidth: number): ChipType[] => {
  let chips: ChipType[] = [];
  const numChips = Math.floor(Math.random() * (maxChips - minChips)) + minChips;

  for (var i = 0; i < numChips; i++) {
    chips.push({
      width: Math.floor(Math.random() * (maxWidth - minWidth)) + minWidth + 'rem',
    });
  }
...
const ListItemSkeleton: React.FC<Props> = (props) => {
  const { numCards = 3, minChips = 2, maxChips = 6, minWidth = 3, maxWidth = 7 } = props;
